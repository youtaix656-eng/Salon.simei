...
test('normalizeState: menus が無い旧データには初期メニューを入れる', async () => {
  const { MENU_SEEDS } = await import('../src/data/menuSeeds.js');
  const state = normalizeState({ clients: [], visits: [] });
  assert.equal(state.menus.length, MENU_SEEDS.length);
  // 空配列は「全部削除した」状態として尊重する
  assert.deepEqual(normalizeState({ clients: [], visits: [], menus: [] }).menus, []);
});

test('MENU_SEEDS: IDが重複せず、カテゴリ順に部分→全身で並ぶ', async () => {
  const { MENU_SEEDS } = await import('../src/data/menuSeeds.js');
  assert.equal(new Set(MENU_SEEDS.map((m) => m.id)).size, MENU_SEEDS.length);
  const groups = groupMenusByCategory(MENU_SEEDS);
  assert.deepEqual(groups.map((g) => g.category), ['部分もみほぐし', '全身もみほぐし']);
  // 全身は時間の昇順（105分が120分より前）
  const full = groups[1].items.map((m) => m.minutes);
  assert.deepEqual(full, [...full].sort((a, b) => a - b));
});
